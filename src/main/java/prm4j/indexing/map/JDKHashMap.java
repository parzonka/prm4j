...
public class JDKHashMap<K, V> extends JDKAbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
    ...
    /**
     * The number of times this MiniMap has been structurally modified Structural modifications are those that change
     * the number of mappings in the MiniMap or otherwise modify its internal structure (e.g., rehash). This field is
     * used to make iterators on Collection-views of the MiniMap fail-fast. (See ConcurrentModificationException).
     */
    transient volatile int modCount;

    /**
     * Constructs an empty <tt>MiniMap</tt> with the specified initial capacity and load factor.
     *
    ...
    /**
     * Constructs an empty <tt>MiniMap</tt> with the specified initial capacity and the default load factor (0.75).
     *
     * @param initialCapacity
     *            the initial capacity.
     * @throws IllegalArgumentException
    ...
    /**
     * Constructs an empty <tt>MiniMap</tt> with the default initial capacity (16) and the default load factor (0.75).
     */
    public JDKHashMap() {
    ...
    /**
     * Constructs a new <tt>MiniMap</tt> with the same mappings as the specified <tt>Map</tt>. The <tt>MiniMap</tt> is
     ...
     * readObject) after MiniMap has been initialized but before any entries have been inserted. (In the absence of this
     ...
     * is critical because MiniMap uses power-of-two length hash tables, that otherwise encounter collisions for
    ...
    /**
     * Returns the entry associated with the specified key in the MiniMap. Returns null if the MiniMap contains no
     * mapping for the key.
     */
    ...
    /**
     * Removes and returns the entry associated with the specified key in the MiniMap. Returns null if the MiniMap
    ...
    /**
     * Returns a shallow copy of this <tt>MiniMap</tt> instance: the keys and values themselves are not cloned.
     *
     ...
     *
     * Subclass overrides this to alter the behavior of MiniMap(Map), clone, and readObject.
     */
    ...
    /**
     * Save the state of the <tt>MiniMap</tt> instance to a stream (i.e., serialize it).
     *
     * @serialData The <i>capacity</i> of the MiniMap (the length of the bucket array) is emitted (int), followed by the
    ...
    /**
     * Reconstitute the <tt>MiniMap</tt> instance from a stream (i.e., deserialize it).
     */
	...
	// Read the keys and values, and put the mappings in the MiniMap
